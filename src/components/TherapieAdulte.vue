<template>
<div>
  <h1 class="hero2 italic">La thérapie pour adultes / séniors</h1>
  <main>
    <br />
    <article>
      <h2 class="italic">
        Vous vous sentez en difficulté dans votre vie personnelle, affective, au
        niveau professionnel, face à un challenge ?
      </h2>
      <br />
      <section class="premier">
        <article class="flex-item w50 bg1">
          <h3>Vous subissez :</h3>
          <ul class="list italic">
            <li>Un évènement traumatique, une agression, un accident…</li>
            <li>Une maladie chronique, psychosomatique ...</li>
            <li>
              Une séparation, la perte d’un proche, un veuvage, l’isolement de
              la retraite
            </li>
            <li>
              Une situation douloureuse, une tristesse excessive, des accès de
              colère incontrôlables
            </li>
            <li>
              Des difficultés professionnelles, burn-out, démission,
              harcèlement…
            </li>
            <li>Une dépression, le sentiment d’être inutile</li>
            <li>De l’anxiété, angoisses, phobies, attaques de panique…</li>
            <li>Un manque de confiance en vous, des doutes existentiels…</li>
            <li>Une addiction (drogue, alcool, jeux, achat, sexe…)</li>
            <li>Des difficultés relationnelles…</li>
          </ul>
        </article>
        <article class="flex-item img w40">
          <img
            src="../../public/assets/Images/chemin.jpg"
            class="hero"
            alt="panneau autoroutier avec des problèmes à gauche et à droite, la psychothérapie au centre"
          />
        </article>
      </section>
      <br />
      <section class="bg1">
        <h3>
          Lors de votre préparation aux concours, examens, entretiens,
          challenges divers, vous aimeriez :
        </h3>
        <ul class="list">
          <li>Augmenter la confiance et l’estime de vous-même</li>
          <li>Apaiser le stress</li>
          <li>Développer votre concentration</li>
          <li>Retrouver vos pleines capacités après une blessure</li>
          <li>Dépasser la peur de gagner ou de perdre…</li>
        </ul>
      </section>
      <br />
      <section class="bg1">
        <h3>
          En vous lançant dans ce voyage thérapeutique vous vous donnez
          l’opportunité de :
        </h3>
        <ul class="list">
          <li>
            Prendre conscience de vos freins, blocages, conditionnements,
            croyances limitantes.
          </li>
          <li>Changer de regard sur les situations.</li>
          <li>
            Vous sentir de plus en plus aligné(e) avec ce que vous êtes
            profondément.
          </li>
        </ul>
      </section>
      <br />
      <section class="bg1">
        <p class="bold">
          Beau challenge en perspective car dans cette transformation, c’est à
          votre nature profonde que vous accédez et c’est votre place dans le
          monde que vous redessinez.
        </p>
        <p>
          La 1ère séance sera l'occasion d’exprimer vos attentes et préfigurer
          votre destination.
        </p>
        <p>
          Votre voyage peut alors débuter, faisons un bout de chemin ensemble.
        </p>
      </section>
    </article>
  </main>
</div>
</template>

<script>
export default {
  name: "TherapieAdulteComponent",
  props: {
    variable1: String, // type de variable (string,integer,...)
  },
  data() {
    return {};
  },
};
</script>

<style>
</style>

